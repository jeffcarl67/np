...
{
    if (to_number_pipe_ or to_user_pipe_) return;
    while (not finished()) {
        pause();
    }
...
{
    processes.remove(p);
    if ((to_number_pipe_ or to_user_pipe_) and processes.empty())
        cmdmgr->cleanup(seqno_);
}
...
class UserPipeIn : public Connection
{
public:
    UserPipeIn(int uid, const std::string& line): uid(uid), line(line) {}
...
void UserPipeIn::connect()
{
    if (processes.size() == 1) {
        processes[0]->set_has_user_pipe_in();
        processes[0]->set_user_pipe_in_uid(uid);
        processes[0]->command()->set_cmdline(line);
    }
}

void UserPipeIn::add_processes(std::vector<std::shared_ptr<Process> >::iterator& iter)
{
    processes.push_back(*iter);
}

class UserPipeOut : public Connection
{
public:
    UserPipeOut(int uid, const std::string& line): uid(uid), line(line) {}
    virtual void connect();
    // this will modify iter
    virtual void add_processes(std::vector<std::shared_ptr<Process> >::iterator& iter);
    virtual ~UserPipeOut() {}
...
void UserPipeOut::connect()
{
    if (processes.size() == 1) {
        processes[0]->set_has_user_pipe_out();
        processes[0]->set_user_pipe_out_uid(uid);
        processes[0]->command()->set_cmdline(line);
    }
}

void UserPipeOut::add_processes(std::vector<std::shared_ptr<Process> >::iterator& iter)
{
    processes.push_back(*iter);
}
...
struct StringGroup
{
    ...
    bool is_redirection();
    bool is_user_pipe_in();
    bool is_user_pipe_out();
    int number();
};
...
bool StringGroup::is_user_pipe_in()
{
    if (strings.size() != 1)
        return false;
    auto& s = strings[0];
...
bool StringGroup::is_user_pipe_out()
{
    if (strings.size() != 1)
        return false;
    auto& s = strings[0];
...
{
    if (is_numbered_pipe() or is_stderr_numbered_pipe() or
        is_user_pipe_in() or is_user_pipe_out())
...
std::shared_ptr<Command> CommandManager::parse(const std::string& line)
{
    ...
    if (token[0] == "setenv") {
        ...
        return std::make_shared<Who>(client, cmdmgr);
    }
    else if (token[0] == "tell") {
        if (token.size() >= 3) {
            auto it = line.begin();
            while (it != line.end() and *it == ' ') ++it;
            for (int i = 0; i < 2; i++) {
                while (it != line.end() and *it != ' ') ++it;
                while (it != line.end() and *it == ' ') ++it;
            }
            int uid = std::stoi(token[1]);
            std::string msg(it, line.end());
            return std::make_shared<Tell>(uid, msg, client, cmdmgr);
        }
        else
            return std::make_shared<Command>(client, cmdmgr);
    }
    else if (token[0] == "yell") {
        if (token.size() >= 2) {
            auto it = line.begin();
            while (it != line.end() and *it == ' ') ++it;
            while (it != line.end() and *it != ' ') ++it;
            while (it != line.end() and *it == ' ') ++it;
            std::string msg(it, line.end());
            return std::make_shared<Yell>(msg, client, cmdmgr);
        }
        else
            return std::make_shared<Command>(client, cmdmgr);
    }
        ...
        if (token.size() >= 2)
            return std::make_shared<Name>(token[1], client, cmdmgr);
        else
            return std::make_shared<Command>(client, cmdmgr);
    }
    else {
        ...
        while (i < groups.size()) {
            ...
            else if (g->is_user_pipe_in()) {
                int n = g->number();
                if (n > 0)
                    conns.push_back(std::make_shared<UserPipeIn>(n, line));
            }
            else if (g->is_user_pipe_out()) {
                int n = g->number();
                if (n > 0)
                    conns.push_back(std::make_shared<UserPipeOut>(n, line));
            }
            else {
        ...
        if (groups.back()->is_user_pipe_out()) {
            cmd->set_to_user_pipe();
        }
