...
public:
    Session(boost::asio::io_context& io_context, const std::string& host,
            ...
            const std::string& socks_host, const std::string& socks_port)
    : resolver_(io_context), socket_(io_context), commands_(),
    host_(host), port_(port), file_(file), index_(index), data_{},
    socks_host_(socks_host), socks_port_(socks_port), data2_{},
    cmd_index_() {}
    ...
    void do_socks_resolve() {
        auto self(shared_from_this());
        resolver_.async_resolve(socks_host_, socks_port_,
    ...
    void do_socks_connect(const boost::asio::ip::tcp::endpoint endpoint) {
        auto self(shared_from_this());
        ...
        resolver_.async_resolve(host_, port_,
            [this, self](boost::system::error_code ec, boost::asio::ip::tcp::resolver::results_type results){
                if (not ec) {
                    data2_[0] = 4;
                    data2_[1] = 1;
                    unsigned short port = std::stoul(port_);
                    data2_[2] = port / 256;
                    data2_[3] = port % 256;
                    auto it = results.cbegin();
                    if (it == results.cend()) { // can not resolve
                        data2_[4] = 0;
                        data2_[5] = 0;
                        data2_[6] = 0;
                        data2_[7] = 1;
                        data2_[8] = 0; // no user name
                        memcpy(data2_ + 9, host_.c_str(), host_.length());
                        data2_[9 + host_.length()] = 0;
                        do_socks_request(9 + host_.length() + 1);
                    }
                    else {
                        auto addr = it->endpoint().address().to_v4().to_bytes();
                        data2_[4] = addr[0];
                        data2_[5] = addr[1];
                        data2_[6] = addr[2];
                        data2_[7] = addr[3];
    ...
    void do_socks_request(std::size_t length) {
        auto self(shared_from_this());
    ...
    void do_socks_response() {
        auto self(shared_from_this());
        boost::asio::async_read(socket_, boost::asio::buffer(data2_, 8),
            [this, self](boost::system::error_code ec, std::size_t length) {
                if (not ec) {
                    if (data2_[1] == 91) {
                        return;
    ...
    void do_write(std::ptrdiff_t n) {
        ...
        if (n > 0 and not commands_.empty()) {
            ...
            commands_.pop_front();
            command += "\n";
            memcpy(data2_, command.c_str(), command.length());
            boost::asio::async_write(socket_, boost::asio::buffer(data2_, command.length()),
...
private:
    ...
    std::string socks_host_, socks_port_;
    unsigned char data2_[max_length];
    int cmd_index_;
};
